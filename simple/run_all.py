"""Grid orchestrator for the simple pipeline.

Runs the 3 × 2 × 10 = 60-cell grid (or a single cell with `--only <run_id>`),
appends each result to `simple/results/leaderboard.csv`, and finally invokes
XAI on the best transformer cell.

Examples:
    python -m simple.run_all
    python -m simple.run_all --only segment_ra_tfidf_svr
    python -m simple.run_all --only raw_ra_dual_gte --epochs 2
...
from simple.data import load_dataframe, split_dataframe
from simple.train import (
    train_classical,
    train_bilstm,
    train_transformer,
)
...
def main():
    ...
    if not args.skip_xai and not args.only:
        try:
            from simple.xai import run_xai_on_best
            run_xai_on_best()
        except Exception as e:
